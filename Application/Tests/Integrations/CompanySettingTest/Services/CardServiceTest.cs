...
public class CardServiceTest: BaseModelsForTests
{
    ...
    public void CreateEntity()
    {
        ...
        var userMadeBy = new SimpleUserMapper().ToDto(context.User.First(u =>
            u.Email.Equals(AddValidCreationCompanyAndUserDto().AddCompanyUser.CreationUserDto!.Email)));
        Assert.NotNull(userMadeBy);
        
        var addSimplifedUser = new SimpleUserMapper().ToDto(context.User.First(u =>
        ...
        Assert.Throws<Exception>(() => cardService.CreateEntity(company, GetBadCardWithEmptyAbsentMemoryAndTaskOrEvent(userMadeBy)));
